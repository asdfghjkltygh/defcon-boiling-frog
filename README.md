...
Cloud red team tools target IAM and service-level vulnerabilities. Pacu (Rhino Security Labs) covers credential theft, privilege escalation, and service exploitation across AWS. Stratus Red Team (Datadog) emulates MITRE ATT&CK techniques for cloud adversary simulation. Neither targets the decision logic of autonomous infrastructure agents; both assume the scaling behavior is opaque and uncontrollable.

Policy-as-code tools (HashiCorp Sentinel, Cloud Custodian) enforce guardrails on scaling policies but don't address the filter pipeline's determinism. Academic work on adversarial ML against control systems (Kurakin et al. 2017, Carlini & Wagner 2017) focuses on classifier evasion, not threshold extraction from infrastructure agents.

The auto-scaler's decision boundary isn't a config you can read from an API; it's an emergent property of the filter pipeline. There's no `aws autoscaling describe-threshold` command. This tool extracts that information via binary search against the pipeline itself.
...
defcon-boiling-frog/
  ...
  BIBLIOGRAPHY.md            Works cited and prior art
  ABSTRACT.txt               Standalone abstract for CFP submission
  OUTLINE.txt                Detailed talk outline for CFP submission
  LICENSE                    MIT
...
See [BIBLIOGRAPHY.md](BIBLIOGRAPHY.md) for the full works-cited list.
